	...
	<view :class="Visible ? 'usable-coupon open' : 'usable-coupon close'">
		...
		<view class="main pt30" v-on:click.stop>
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'">
				<view class="p-0-30">
					<view @click="selectCoupon(item.userCouponId)" style="margin-bottom: 15rpx;"
						:class="'coupon-item coupon-item-'+item.colorText" v-for="(item, index) in datalist"
						:key="index">
						<!--装饰用的小圆-->
						<view class="circles"><text v-for="(circle, num) in 6" :key="num"></text></view>
						<view class="info w100">
							<view class="d-c-c d-c">
								<text class="f40 fb w-s-n">{{ item.couponTypeText }}</text>
							</view>
						</view>
							...
							<view class="flex-1 o-h">
								<view class="f34">
									<template v-if="item.couponType == 10">
										<text class="f40">减{{ item.reducePrice*1 }}元</text>
									</template>
								...
								<view class=" f24">{{item.minPrice>0?'满'+item.minPrice*1+'元可用':'无门槛'}}</view>
								<view class="mt30 f24 white">{{ item.startTimeText }}~{{ item.endTimeText }}</view>
							</view>
