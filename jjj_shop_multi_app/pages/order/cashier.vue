...
export default {
	...
	methods: {
		back() {
			...
			uni.showModal({
				...
				success: function(res) {
					if (res.confirm) {
						uni.reLaunch({
							url:'/pages/order/order-detail?orderId=' + self.orderId
						})
						console.log('用户点击离开');
					} else if (res.cancel) {
						console.log('用户点击取消');
		...
		paySuccess(result) {
			let self = this;
			if (self.orderType == 30 || self.orderType == 70) {
				self.navigateBack();
		...
		payError(result) {
			...
			if (self.orderType == 50 || self.orderType == 40 || self.orderType == 30 || self.orderType == 20 || self.orderType == 70) {
				self.navigateBack();
			} else if (self.orderType == 60) {
