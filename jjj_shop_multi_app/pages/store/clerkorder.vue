<template>
	<view class="order-datail">
		...
		<!-- 上门自提：自提门店 -->
		<view class="order-express p30 d-s-c" v-if="detail.deliveryType == 20">
			...
			<view class="list">
				<view class="one-product p-20-0" v-for="(item, index) in detail.product" :key="index">
					<view class="d-s-c">
						...
						<view class="flex-1">
							<view class="pro-info">{{ item.productName }}</view>
		...
		<view class="group bg-white f28">
			...
			<view class="p-20-0 d-e-c fb f34">
				应付金额：
				<text class="red">¥ {{ detail.orderPrice }}</text>
			</view>
		</view>
		...
		<view v-if="detail.orderStatus != 20 " class="flow-fixed-footer b-f">
			<!-- 订单核销 -->
			<view v-if="detail.payStatus==20 && detail.deliveryType ==20 && detail.deliveryStatus == 10 ">
				<button class="btn-red" @click="onSubmitExtract(detail.orderId)">确认核销</button>
			</view>
		</view>
	</view>
</template>
