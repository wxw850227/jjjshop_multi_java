				...
				<view class="level-box count_choose">
					...
					<view class="d-s-c">
						...
						<view class="text-wrap">
							<input type="text" v-model="form.showSku.sum"  />
						</view>
	...
	export default {
			...
			'isPopup': function(n, o) {
				if (n != o) {
					...
					if (!this.isOpenSpec) {
						...
						this.roomId = this.productModel.roomId;
		...
		methods: {
			...
			createdOrder() {
				...
				if(this.form.detail.specType == 20){
					productSkuId = this.form.productSpecArr.join("_");
				}
				let roomId = this.roomId;
				let url = '/pages/order/confirm-order?productId=' + productId + '&productNum=' + productNum +
					'&specSkuId=' + productSkuId + '&orderType=buy'+'&roomId=' + roomId;
				this.gotoPage(url);
			},
