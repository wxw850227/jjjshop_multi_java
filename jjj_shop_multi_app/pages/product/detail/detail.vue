	...
	<view class="product-detail pr" :data-theme="theme()" :class="theme() || ''">
		...
		<scroll-view scroll-y="true" class="scroll-Y scroll-box" :style="'height:' + scrollviewHigh + 'px'"
				...
				<swiper class="swiper" indicator-active-color="#ffffff" indicator-color="rgba(255,255,255,.3)"
					...
					<swiper-item v-if="detail.videoId != 0">
						...
						<image v-if="!isVideoPlay"
							:src="detail.posterId != 0 ? detail.posterFilePath : detail.image[0].filePath" mode=""
			...
			<template v-if="isPreview == 1">
				<previewProduct v-if="!loadding" :detail="detail" @send="sendFunc" :isFav="isFav"></previewProduct>
			</template>
			<!--基本信息-->
			<template v-if="ispresale">
				...
				<view class="limited-spike m-0-20">
					<view class="d-s-c mb16">
						...
						<view class="tips-box">
							预估到手价￥{{ (detail.advance.sku[0].product_price * 1 - detail.advance.sku[0].advance_price * 1).toFixed(2) }}
						</view>
					</view>
					<text class="left-name">定金¥{{ detail.advance.money }}
						(尾款立减¥{{ detail.advance.sku[0].advance_price }})</text>
						...
						<view class="d-b-s">
							...
							<view class="flex-1">
								<view class="f24 redF11">
									￥{{ (detail.advance.sku[0].product_price * 1 - detail.advance.sku[0].advance_price * 1).toFixed(2) }}
								</view>
			...
			<view class="bg-white p30 mb22" v-if="!ispresale && isPreview != 1">
			...
			<view class="o-h p-0-30 bg-white mb21">
				<!--已选择-->
				<view class="already-choice" v-if="detail.spec_type == 20"
		...
		<view class="btns-wrap">
			...
			<view v-if="store_open && shop_info.userId != userId" class="icon-box d-c-c" @click="openService">
				<button class="d-c-c d-c bg-white">
		...
		<!--客服-->
		<Mpservice v-if="isMpservice" :shopSupplierId="shop_supplier_id" :isMpservice="isMpservice"
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.userId = uni.getStorageInfoSync('userId');
			this.roomId = e.roomId || 0;
		},
		mounted() {
			this.init();
			/*获取产品详情*/
			...
			getData() {
				...
				self._get(
					...
					function(res) {
						...
						console.log(self.shop_supplier_id)
						...
						self.serviceType = res.data.serviceType;
						self.getServer();
						self.loadding = false;
						uni.hideLoading();
					}
