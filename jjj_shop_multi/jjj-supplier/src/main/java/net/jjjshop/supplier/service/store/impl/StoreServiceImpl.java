...
public class StoreServiceImpl extends BaseServiceImpl<StoreMapper, Store> implements StoreService {
    ...
    /**
     * 商家门店分页查询
     * @param storePageParam
     * @return
     */
    public Paging<StoreVo> getList(StorePageParam storePageParam) {
        Page<Store> page = new PageInfo(storePageParam);
        LambdaQueryWrapper<Store> wrapper = new LambdaQueryWrapper<>();
        if(storePageParam.getStatus() != null) {
            wrapper.eq(Store::getStatus, storePageParam.getStatus());
        }
        if(storePageParam.getShopSupplierId() != null) {
            wrapper.eq(Store::getShopSupplierId, storePageParam.getShopSupplierId());
        }
        wrapper.eq(Store::getIsDelete, 0);
     ...
     * @return
     */
    public List<StoreVo> getAll(Integer shopSupplierId) {
        List<Store> list = this.list(new LambdaQueryWrapper<Store>()
                .eq(Store::getShopSupplierId, shopSupplierId)
