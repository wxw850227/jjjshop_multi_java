...
import net.jjjshop.supplier.param.setting.DeliveryPageParam;
import net.jjjshop.supplier.param.setting.DeliveryParam;
import net.jjjshop.supplier.service.product.ProductService;
import net.jjjshop.supplier.service.settings.DeliveryRuleService;
import net.jjjshop.supplier.service.settings.DeliveryService;
import net.jjjshop.supplier.vo.setting.DeliveryVo;
...
public class DeliveryServiceImpl extends BaseServiceImpl<DeliveryMapper, Delivery> implements DeliveryService {
    ...
    public Paging<DeliveryVo> getList(DeliveryPageParam deliveryPageParam) {
        ...
        IPage<Delivery> iPage = this.page(page, new LambdaQueryWrapper<Delivery>()
                .eq(Delivery::getShopSupplierId, deliveryPageParam.getShopSupplierId())
                .orderByAsc(Delivery::getSort));
        // 最终返回分页对象
        IPage<DeliveryVo> resultPage = iPage.convert(item -> {
            DeliveryVo vo = new DeliveryVo();
            BeanUtil.copyProperties(item, vo);
            vo.setMethodText(vo.getMethod() == 10 ? "按件数":"按重量");
