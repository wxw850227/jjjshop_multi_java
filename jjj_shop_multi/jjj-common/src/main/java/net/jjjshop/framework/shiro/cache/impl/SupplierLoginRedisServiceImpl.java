 ...
 **/
@Service
public class SupplierLoginRedisServiceImpl implements SupplierLoginRedisService {
     ...
     */
    @Override
    public void cacheLoginInfo(JwtToken jwtToken, LoginSupplierUserVo loginSupplierUserVo) {
        if (jwtToken == null) {
            throw new IllegalArgumentException("jwtToken不能为空");
        }
        if (loginSupplierUserVo == null) {
            throw new IllegalArgumentException("loginSupplierUserVo不能为空");
        }
        // token
        ...
        // 登录用户名称
        String username = loginSupplierUserVo.getUserName();
        // token md5值
        ...
        LoginSupplierUserRedisVo loginSupplierUserRedisVo = new LoginSupplierUserRedisVo();
        BeanUtils.copyProperties(loginSupplierUserVo, loginSupplierUserRedisVo);
        loginSupplierUserRedisVo.setSalt(salt);
        loginSupplierUserRedisVo.setClientInfo(clientInfo);
        ...
        // 1. tokenMd5:jwtTokenRedisVo
        String loginTokenRedisKey = String.format(CommonRedisKey.SUPPLIER_LOGIN_TOKEN, tokenMd5);
        redisTemplate.opsForValue().set(loginTokenRedisKey, jwtTokenRedisVo, expireDuration);
        // 2. username:loginSysUserRedisVo
        redisTemplate.opsForValue().set(String.format(CommonRedisKey.SUPPLIER_LOGIN_USER, username), loginSupplierUserRedisVo, expireDuration);
        // 3. salt hash,方便获取盐值鉴权
        redisTemplate.opsForValue().set(String.format(CommonRedisKey.SUPPLIER_LOGIN_SALT, username), salt, expireDuration);
        // 4. login user token
        redisTemplate.opsForValue().set(String.format(CommonRedisKey.SUPPLIER_LOGIN_USER_TOKEN, username, tokenMd5), loginTokenRedisKey, expireDuration);
    }

    @Override
    public void refreshLoginInfo(String oldToken, String username, JwtToken newJwtToken) {
        // 获取缓存的登录用户信息
        LoginSupplierUserRedisVo loginSupplierUserRedisVo = getLoginSupplierUserRedisVo(username);
        // 删除之前的token信息
        deleteLoginInfo(oldToken, username);
        // 缓存登录信息
        cacheLoginInfo(newJwtToken, loginSupplierUserRedisVo);
    }

    @Override
    public LoginSupplierUserRedisVo getLoginSupplierUserRedisVo(String username) {
    ...
    @Override
    public LoginSupplierUserVo getLoginSupplierUserVo(String username) {
        if (StringUtils.isBlank(username)) {
            throw new IllegalArgumentException("username不能为空");
        }
        LoginSupplierUserRedisVo userRedisVo = getLoginSupplierUserRedisVo(username);
        return userRedisVo;
    }
    ...
    public String getSalt(String username) {
        ...
        String salt = (String) redisTemplate.opsForValue().get(String.format(CommonRedisKey.SUPPLIER_LOGIN_SALT, username));
        return salt;
    }
    ...
    public boolean exists(String token) {
        ...
        String tokenMd5 = DigestUtils.md5Hex(token);
        Object object = redisTemplate.opsForValue().get(String.format(CommonRedisKey.SUPPLIER_LOGIN_TOKEN, tokenMd5));
        return object != null;
    }

    @Override
    public void deleteUserAllCache(String username) {
        Set<String> userTokenMd5Set = redisTemplate.keys(String.format(CommonRedisKey.SUPPLIER_LOGIN_USER_ALL_TOKEN, username));
