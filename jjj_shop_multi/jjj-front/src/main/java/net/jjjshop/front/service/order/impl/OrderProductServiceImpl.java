    ...
    public List<OrderProductVo> getProductVoList(Integer orderId) {
        return this.list(new LambdaQueryWrapper<OrderProduct>().eq(OrderProduct::getOrderId, orderId))
                .stream().map(e -> {
                    ...
                    if (orderRefundService.count(new LambdaQueryWrapper<OrderRefund>()
                            .eq(OrderRefund::getOrderProductId, vo.getOrderProductId())
                            .eq(OrderRefund::getOrderId, orderId)) > 0) {
                        vo.setRefund(true);
                    } else {
                        vo.setRefund(false);
                    }
