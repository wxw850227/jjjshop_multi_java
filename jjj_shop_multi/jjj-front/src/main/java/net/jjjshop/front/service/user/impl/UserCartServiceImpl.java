...
public class UserCartServiceImpl extends BaseServiceImpl<UserCartMapper, UserCart> implements UserCartService {
     ...
     * @return
     */
    public List<JSONObject> getAll(User user) {
        ...
        // 按店铺分类
        Map<Integer, List<ProductCartVo>> subListMap = voList.stream().collect(Collectors.groupingBy(ProductCartVo::getShopSupplierId));
        List<JSONObject> list = new ArrayList<>();
        subListMap.forEach((key, value) -> {
            JSONObject item = new JSONObject();
            item.put("productList", value);
            Supplier supplier = supplierService.getById(key);
            SupplierVo vo = new SupplierVo();
            BeanUtils.copyProperties(supplier, vo);
            item.put("shopSupplierId", key);
            item.put("supplier", vo);
            list.add(item);
        });
        return list;
    }
    ...
    public Boolean add(User user, Integer productId, String specSkuId, Integer totalNum) {
        ...
        if (cart != null) {
            ...
            saveBean.setShopSupplierId(product.getShopSupplierId());
