...
public class UserGradeServiceImpl extends BaseServiceImpl<UserGradeMapper, UserGrade> implements UserGradeService {
    ...
    private void setRemark(UserGradeParam userGradeParam) {
        ...
        if (userGradeParam.getOpenPoints() == 1) {
            if (remark != null) {
                remark.append(System.getProperty("line.separator"));
            }
            remark.append("会员积分满" + userGradeParam.getUpgradePoints() + "可升级到此等级");
        }
        if (userGradeParam.getOpenInvite() == 1) {
            if (remark != null) {
                remark.append(System.getProperty("line.separator"));
            }
            remark.append("会员邀请人数满" + userGradeParam.getUpgradeInvite() + "可升级到此等级");
        }
