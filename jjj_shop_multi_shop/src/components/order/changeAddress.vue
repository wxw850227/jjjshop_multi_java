<template>
  <el-dialog title="查看物流" v-if="areaList != ''" v-model="dialogVisible" :close-on-click-modal="false" :close-on-press-escape="false" width="900px">
    ...
    <el-select class="mb16 ml10" v-if="addressData.provinceId != ''" v-model="addressData.cityId" placeholder="市" @change="initRegion">
      <el-option :label="item1.name" :value="item1.id" v-for="(item1, index1) in cityList" :key="index1"></el-option>
    </el-select>
    <el-select class="mb16 ml10" v-if="addressData.cityId != ''" v-model="addressData.regionId" placeholder="区">
